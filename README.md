...
| `GET/POST/PUT/DELETE` | `/v1/admin/whatsapp-flows` | (scope) **Formularios** (WhatsApp Flows de Meta, no confundir con los flujos de arriba): crear borrador (`POST /{waba_id}/flows`) y subir el Flow JSON como asset (`/{flow_id}/assets`, devuelve los `validation_errors`), `PUT /{id}/json`, `/refresh` (estado + URL de vista previa), `/publish`, `/deprecate`, `DELETE` (solo borradores), `/sync`, `/validate` (solo el validador local), `/from-library` (plantillas listas, p.ej. `confirm_booking`) y `/generate` (IA vía nexolu-ia-core: valida local y contra Meta, hasta 2 reintentos con los errores). Ver core/whatsapp_flows/. |
